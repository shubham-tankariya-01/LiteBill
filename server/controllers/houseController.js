...
export const getHouses = async (req, res, next) => {
    try {
        const houses = await House.find({ user_id: req.session.userId });
...
// GET /houses/new
export const getNewHouseForm = (req, res) => {
...
// GET /houses/:houseId/edit
export const getEditHouseForm = async (req, res, next) => {
    try {
        const house = await House.findOne({ 
            _id: req.params.houseId, 
            user_id: req.session.userId 
...
export const updateHouse = async (req, res, next) => {
    try {
        ...
        const house = await House.findOneAndUpdate(
            { _id: req.params.houseId, user_id: req.session.userId }, 
            { house_name }
        );
        if (!house) return res.status(404).send('Resource not found');
...
export const deleteHouse = async (req, res, next) => {
    try {
        ...
        if (cycleIds.length > 0) {
            ...
            await RoomReading.deleteMany({ billing_cycle_id: { $in: cycleIds } });
        }
...
export const getHouseHistory = async (req, res, next) => {
    try {
        const { houseId } = req.params;
        const house = await House.findOne({ 
            _id: houseId, 
